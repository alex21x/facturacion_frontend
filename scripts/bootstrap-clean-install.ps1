param(
    ...
    [string]$FrontendBranch = "docker-multi-entorno",
    [string]$BackendBranch = "docker-multi-entorno"
)
...
function Ensure-Repository {
    ...
    foreach ($branch in $normalizedCandidates) {
        ...
        git clone --branch $branch --single-branch $RepoUrl $TargetPath | Out-Null
        if ($LASTEXITCODE -eq 0) {
            return
        }
...
function Resolve-InstallScriptPath {
    ...
    $primaryCandidate = Join-Path $InstallerScriptsPath "install-local.ps1"
    ...
    $patternCandidate = Get-ChildItem -Path $InstallerScriptsPath -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like "install-local*.ps1" -or $_.Name -like "setup-local*.ps1" } |
        Select-Object -First 1
    if ($patternCandidate) {
        return $patternCandidate.FullName
    }

    $frontendCandidate = Join-Path $ClonedFrontendPath "scripts\install-local.ps1"
...
Ensure-GitAvailable
Ensure-Repository -TargetPath $targetFrontendRoot -RepoUrl $FrontendRepoUrl -BranchCandidates @($FrontendBranch, "docker-multi-entorno", "feature/docker-multientorno") -Name "frontend"
Ensure-Repository -TargetPath $targetBackendRoot -RepoUrl $BackendRepoUrl -BranchCandidates @($BackendBranch, "docker-multi-entorno", "feature/docker-multientorno") -Name "backend"
Ensure-FrontendDockerBranch -FrontendPath $targetFrontendRoot -CandidateBranches @($FrontendBranch, "docker-multi-entorno", "feature/docker-multientorno")
Ensure-BackendDockerBranch -BackendPath $targetBackendRoot -CandidateBranches @($BackendBranch, "docker-multi-entorno", "feature/docker-multientorno")

$installerScriptsRoot = Resolve-Path $PSScriptRoot -ErrorAction SilentlyContinue
if (-not $installerScriptsRoot) {
    throw "No se pudo resolver la carpeta scripts del instalador."
}

$installScript = Resolve-InstallScriptPath -InstallerScriptsPath $installerScriptsRoot.Path -ClonedFrontendPath $targetFrontendRoot
$composeFile = Join-Path $targetFrontendRoot "docker-compose.local.yml"

if (-not $installScript) {
    $filesInInstallerScripts = (Get-ChildItem -Path $installerScriptsRoot.Path -File -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name) -join ", "
    if ([string]::IsNullOrWhiteSpace($filesInInstallerScripts)) {
        $filesInInstallerScripts = "(sin archivos visibles)"
    }

    throw "No se encontro script instalador (setup-local.ps1 o install-local.ps1). Ruta de scripts: $($installerScriptsRoot.Path). Archivos detectados: $filesInInstallerScripts"
}
